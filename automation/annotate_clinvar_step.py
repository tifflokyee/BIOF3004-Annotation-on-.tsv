...
from auto_annotate_generated import annotate_clinvar, default_output_path, load_variant_table


def main():
    parser = argparse.ArgumentParser(description="Step 4: add ClinVar VCF annotations.")
    ...
    print("=== Step: ClinVar annotation ===")
    print(f"Input: {args.input}")
    df = load_variant_table(args.input)
    before_columns = set(df.columns)
    df = annotate_clinvar(df, Path(args.annotation_dir))
    added_columns = [column for column in df.columns if column not in before_columns]
    matched = (df["clinvar_vcf_id"] != ".").sum()
    df.to_csv(output, sep="\t", index=False)
    print(f"Rows annotated by ClinVar: {matched:,} / {len(df):,}")
